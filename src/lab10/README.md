...
## Стек( (Stack)) работает по принципу LIFO (последний вошел, первый вышел), очередь — по принципу FIFO (первый вошел, первый вышел), а связанный список представляет собой цепочку узлов, где каждый элемент связан со следующим. Хотя списки обеспечивают гибкость в управлении памятью, стеки и очереди часто реализуются на их основе или на основе массивов для решения конкретных задач, таких как управление вызовами функций или обработка потоков данных.

## Операции:
## push(x) — положить элемент сверху;
## pop() — снять верхний элемент;
## peek() — посмотреть верхний, не снимая.  Типичные применения:
